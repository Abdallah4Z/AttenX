...
\begin{itemize}
    \item We design and implement a \textbf{Non-Local Self-Attention} module within the multi-stage generator. By injecting this module after the $128 \times 128$ resolution stage, the model can compute a weighted response across all spatial positions, explicitly enforcing global coherence without sacrificing computational efficiency.
...
Given an intermediate feature map $X \in \mathbb{R}^{C \times H \times W}$ from the $128 \times 128$ stage ($F_1$), we flatten the spatial dimensions to $X \in \mathbb{R}^{C \times N}$, where $N = H \times W$. The feature map is linearly projected into three spaces: Query ($Q$), Key ($K$), and Value ($V$):
\begin{equation}
...
AttnGAN utilizes three discriminators ($D_0$ at $64 \times 64$, $D_1$ at $128 \times 128$, and $D_2$ at $256 \times 256$). To prevent these networks from easily distinguishing generated distributions from real ones early in training, we apply Spectral Normalization.
