...
if [[ -d "train" && -d "test" ]]; then
	...
	# If metadata was extracted into birds/ instead of train/test directly,
	# run the preprocessor to restructure it.
	if [[ -d "birds" && ! -d "train" ]]; then
		echo "Restructuring metadata from birds/ into train/ and test/ ..."
		cd ".."
		python3 scripts/preprocess_cub_metadata.py "$DIR"
		cd "$DIR"
	fi
