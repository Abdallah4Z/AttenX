...
Evaluation on the CUB-200-2011 dataset (quantitative metrics to be reported after full training) demonstrates the architectural validity of the proposed framework. All source code and pretraining scripts are available at \url{https://github.com/AttenX}.
\end{abstract}
...
\begin{figure*}[htbp]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2em}\textbf{[Architecture Diagram Placeholder]}\\
\textit{Illustrate: Text Encoder $\rightarrow$ CA ($\mu$, $\log\sigma$, KL) $\rightarrow$ stage0 (noise + CA) $\rightarrow$ stages 1-5 (upsample + cross-attention, stage 4 has self-attention) $\rightarrow$ stage6 (plain upsample) $\rightarrow$ multi-scale RGB outputs (64, 128, 256) $\rightarrow$ D64, D128, D256 with spectral norm.}\vspace{2em}}}
...
We report two standard metrics for text-to-image synthesis:
\begin{itemize}
    \item \textbf{Inception Score (IS):} Measures image quality and diversity. Higher is better.
    \item \textbf{Fr\'{e}chet Inception Distance (FID):} Measures distributional distance between generated and real images. Lower is better.
\end{itemize}

\textbf{Note:} The following results are to be measured after full model training. The evaluation framework (`scripts/evaluate\_metrics.py`) computes IS using Inception-v3 and supports FID given pre-computed real dataset statistics.

\begin{table}[htbp]
\caption{Quantitative results on CUB-200-2011 (to be measured after training)}
\centering
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Model} & \textbf{IS} ($\uparrow$) & \textbf{FID} ($\downarrow$) \\
\midrule
AttnGAN (reported) & 4.36 & 23.98 \\
\textbf{AttenX (Ours)} & \textbf{[To be measured]} & \textbf{[To be measured]} \\
\bottomrule
\end{tabular}
\label{tab:results}
\end{table}
...
To isolate the contribution of each architectural component, we define four configurations that can be evaluated using the codebase:
...
\begin{table}[htbp]
\caption{Ablation study on CUB-200-2011 (to be measured after training)}
\centering
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Configuration} & \textbf{IS} ($\uparrow$) & \textbf{FID} ($\downarrow$) \\
\midrule
M1: Baseline (AttnGAN-style) & [To be measured] & [To be measured] \\
M2: + Self-Attention & [To be measured] & [To be measured] \\
M3: + Spectral Norm & [To be measured] & [To be measured] \\
\textbf{M4: AttenX (Full)} & \textbf{[To be measured]} & \textbf{[To be measured]} \\
\bottomrule
\end{tabular}
\label{tab:ablation}
\end{table}
...
Figure \ref{fig:comparison} shows qualitative comparisons between AttnGAN and AttenX on sample captions. AttenX is expected to exhibit improved global anatomical coherence (e.g., consistent head-to-tail proportions) and sharper fine-grained details due to the deeper cross-attention pipeline.

\begin{figure}[htbp]
\centering
\fbox{\parbox{0.8\columnwidth}{\centering\vspace{1em}\textbf{[Qualitative comparison figure]}\\\textit{To be generated after training}\vspace{1em}}}
\caption{Qualitative comparison between baseline AttnGAN (left) and AttenX (right). AttenX is expected to show improved global coherence and fewer anatomical artifacts.}
