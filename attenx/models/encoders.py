...
class RNN_ENCODER(nn.Module):
    ...
    Takes word indices, embeds them, and runs through a
    bidirectional LSTM. Returns word-level features and a
    global sentence embedding (concatenation of final
    hidden states).
    """
    ...
    def forward(self, captions, cap_lens, hidden=None):
        ...
        packed = nn.utils.rnn.pack_padded_sequence(
            embeddings, cap_lens.cpu(), batch_first=True,
...
class CNN_ENCODER(nn.Module):
    ...
    Uses a pretrained Inception-v3 (frozen) to extract local
    feature maps and global image codes. A 1x1 conv projects
    local features to nef dim, and a linear layer projects the
    global pool to nef dim.
    """
