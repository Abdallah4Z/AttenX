...
class D_GET_LOGITS(nn.Module):
    """Conditional logit branch for the multi-scale discriminators.

    Concatenates the sentence embedding (spatially replicated)
    with the image features and produces a single scalar logit.
    """
...
class D_NET64(nn.Module):
    ...
    Downsampling: 64 → 32 → 16 → 8 → 4 (ndf*8 feature map).
    Conditional logits via D_GET_LOGITS.
    """
...
class D_NET128(nn.Module):
    ...
    Downsampling: 128 → 64 → 32 → 16 → 8 → 4 (ndf*8 feature map).
    """
...
class D_NET256(nn.Module):
    ...
    Downsampling: 256 → 128 → 64 → 32 → 16 → 8 → 4 (ndf*8).
    """
