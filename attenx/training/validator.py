...
@torch.no_grad()
def validate(netG, netsD, text_encoder, image_encoder, val_loader,
             device, config):
    """Run validation for one epoch.

    Returns mean D loss, G loss, DAMSM loss, and KL loss.
    """
    netG.eval()
    for netD in netsD:
        netD.eval()
    ...
    for batch in val_loader:
        ...
        words_emb, sent_emb = text_encoder(captions_sorted,
                                           cap_lens_sorted, None)
        sent_emb_d = sent_emb.detach()
        word_emb_d = words_emb.detach()

        real_64 = nn.functional.interpolate(real_imgs, size=(64, 64),
                                            mode="bilinear",
                                            align_corners=False)
        real_128 = nn.functional.interpolate(real_imgs, size=(128, 128),
                                             mode="bilinear",
                                             align_corners=False)
        real_scales = [real_64, real_128, real_imgs]

        noise = torch.randn(batch_size, nz, 1, 1, device=device)
        img_64, img_128, img_256, mu, logvar = netG(noise, sent_emb_d,
                                                     word_emb_d)
        ...
        noise = torch.randn(batch_size, nz, 1, 1, device=device)
        img_64, img_128, img_256, mu, logvar = netG(noise, sent_emb,
                                                     word_emb_d)
        fake_scales = [img_64, img_128, img_256]
