...
def words_loss(img_features, words_emb, labels, cap_lens, batch_size):
    """Word-level DAMSM loss.

    For each word, attends over spatial image features and
    computes cosine similarity between attended vector and
    the word embedding. Then uses cross-modal contrastive loss.
    """
...
def sent_loss(cnn_code, sent_emb, labels, batch_size):
    """Sentence-level DAMSM loss (contrastive ranking).

    Maximizes similarity between correct image-sentence pairs
    and minimizes similarity with all incorrect pairs in batch.
    """
